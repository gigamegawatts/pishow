#!/usr/bin/env python3
"""
SHT31 Temperature and Humidity Sensor Test Utility
Interfaces with an SHT31 sensor over I2C to read temperature and humidity.
"""
...
# SHT31 Command: Clock stretching disabled, High repeatability
CMD_MEAS_HIGH_REP = bytes([0x24, 0x00])

def crc8(data: bytes) -> int:
    """Calculates the SHT31 CRC-8 checksum.
...
class SHT31:
    ...
    def read(self, verify_crc: bool = True):
        """Triggers SHT31 conversion and returns (temp_c, humidity)."""
        if self.fd is not None:
            # Raw file descriptor read/write
            try:
                os.write(self.fd, CMD_MEAS_HIGH_REP)
                # SHT31 high repeatability conversion takes max 15ms. Sleep 50ms to be safe.
                time.sleep(0.05)
            ...
            # register which SHT31 doesn't use for reads, we use raw I2C messages if smbus2 is used,
...
def main():
    parser = argparse.ArgumentParser(
        description="SHT31 I2C Temperature & Humidity Sensor Utility",
    ...
    # Initialize sensor
    try:
        sensor = SHT31(bus_num=args.bus, address=address_val, method=args.method)
    except Exception as e:
        print(f"Error connecting to SHT31 sensor:\n{e}", file=sys.stderr)
        sys.exit(1)

    if not args.json:
        print("SHT31 I2C Sensor Utility")
        print("========================")
